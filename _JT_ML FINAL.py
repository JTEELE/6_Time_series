...
ticker_ls = []
sktime_predictions = []
scalecast_predictions = []

def load_and_preprocess_data(time_series, prediction_days):
    df = time_series.copy()
...
def generate_ml_models(time_series, prediction_days, name):
    X_train, y_train, scaler, df = load_and_preprocess_data(time_series, prediction_days)
...
def scalecast_model(dataframe, prediction_days):
    f = Forecaster(
    y = dataframe['values'], # required
    current_dates = dataframe['date'], # required
    future_dates = prediction_days, # length of the forecast horizon
    ...
    # print(results)
    return list(results.values())[-1]['xgboost'].iloc[-1]

if __name__ == "__main__":
    time_series = pd.read_csv(r"C:\\Users\\jteel\\Documents\\Github\\Alpaca\\data\\historical_prices.csv")
    time_series = time_series.set_index(time_series.columns[0])[["t","VWMA"]]
    time_series.columns = ['date', 'values']
    prediction_days = 10

    tickers = time_series.index.unique()
    counter = len(tickers)
    for i in range(len(tickers)):
    # for i in range(3):
        name = tickers[i]
        counter -= 1
        print(f"{counter}: {tickers}")
        try:
            ticker_time_series = time_series[time_series.index == name].copy()
            sktime_pred = generate_ml_models(ticker_time_series, prediction_days, name)
            scalecast_pred = scalecast_model(ticker_time_series, prediction_days)
            ticker_ls.append(name)
            sktime_predictions.append(sktime_pred)
            scalecast_predictions.append(scalecast_pred)
        except:
            print(f"Error: {name}")
    
    print(ticker_ls)
    print(sktime_predictions)
    print(scalecast_predictions)
    pd.DataFrame({'Ticker': ticker_ls, 'Sktime': sktime_predictions, 'Scalecast': scalecast_predictions}).to_csv('ML/ML_RESULTS.csv')